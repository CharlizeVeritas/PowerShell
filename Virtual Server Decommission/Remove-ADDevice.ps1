<#
  .SYNOPSIS
  Removes a Device Record from Active Directory.

  .DESCRIPTION

  This script will remove a Device Record from Active Directory. If the script
  ...
  .OUTPUTS
  Remove-ADDevice.ps1 will output any Exception Messages if they occur.

  .EXAMPLE
  PS> .\Remove-ADDevice.ps1 -username XXXXX -password Password1! -JumpServer XXXXXXXXXX.domain.com -vmhostname YYYYY -ADUser ZZZZZ -ADPass Password1!
#>
...
# Output Results
If (!$Device) {return "WARNING: No ObjectGUID found for $vmhostname in Active Directory on Domain $Domain (Please check manually)"}

# Attempt Removal without -Recursive
Try {Remove-ADComputer -Identity $vmhostname -Credential $Credential -Confirm:$False -ErrorAction Stop}
Catch [System.ServiceModel.FaultException] {
    
    # Attempt Removal without -Recursive
    Try {Get-ADComputer -Identity $vmhostname -Properties * -Credential $Credential -ErrorAction Stop | Remove-ADObject -Credential $Credential -Recursive -Confirm:$False -ErrorAction Stop}
    Catch {throw "FAILED: Error occured removing $vmhostname in Active Directory on Domain $($Domain): $($_.Exception.Message)"}

    }
Catch {throw "FAILED: Error occured removing $vmhostname in Active Directory on Domain $($Domain): $($_.Exception.Message)"}

# Verify Device Object does not exist in AD
Try {$Test = Get-ADComputer -Identity $vmhostname -Properties * -Credential $Credential -ErrorAction Stop}
Catch {$Test = $null} # This is to completely supress any errors from Get-ADComputer when device not found

# Return Results
If ($Test) {Throw "FAILED: Error occured removing $vmhostname in Active Directory on Domain $($Domain) (Device Still Exists)"} else {return "SUCCESS: Decommissioned ObjectGUID $($Device.ObjectGUID) for $vmhostname on Domain $Domain"} 

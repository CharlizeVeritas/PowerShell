<#
  .SYNOPSIS
  Removes an IB Host Record from the Infoblox Server DNS table.

  .DESCRIPTION
  This script will remove an IB Host Record from the Infoblox Server. If the script
  ...
  .OUTPUTS
  Remove-IBHostRecord.ps1 will output any Exception Messages if they occur.

  .EXAMPLE
  PS> .\Remove-IBHostRecord.ps1 -InfoBloxServer YYYYY.domain.com -vmhostname YYYYY -InfoBloxUser ZZZZZ -InfoBloxPass Password1!
#>
...
If ((!$refid) -or ($refid -eq $null) -or ($refid -eq "")) {return "WARNING: No Reference ID found for $($vmhostname) on InfoBlox Server $InfoBloxServer (Please check manually)"}

# Delete Record Based on Reference ID
Try {$webrequest = Invoke-RestMethod -Credential $Credential -Uri "https://$InfoBloxServer/wapi/v2.0/record:host/$refid"  -Method DELETE -ErrorAction Stop}
Catch {throw "FAILED: Error occured in deleting Reference ID $refid for $recordType Record $($vmhostname) with IP $ipaddress on InfoBlox Server $($InfoBloxServer): $($_.Exception.Message)"}

# Check Host Record for Verification
Try {$HostRecordTest = Invoke-WebRequest -Credential $Credential -Uri "https://$InfoBloxServer/wapi/v2.10.5/record:host?name:=$vmhostname" -Method Get -ErrorAction Stop}
Catch {throw "FAILED: Error occured in getting Host Record for $($vmhostname) / $($_.Exception.Message)"}

# Check A Record for Verification
Try {$ARecordTest = Invoke-WebRequest -Credential $Credential -Uri "https://$InfoBloxServer/wapi/v2.10.5/record:a?name:=$vmhostname" -Method Get -ErrorAction Stop}
Catch {throw "FAILED: Error occured in getting A Record for $($vmhostname) / $($_.Exception.Message)"}

# Get Host Record Information
If (($HostRecordTest.Content) -and ($HostRecordTest.Content -ne "[]")) {

    # Get Reference ID for Host Record and IP Address
    Try {$jsonContent = $HostRecordTest.Content | ConvertFrom-Json
    $jsonContent._ref -match "record:host/([^;]*):" | Out-Null
    $refnew = $Matches[1]
    $jsonContent = $jsonContent | select @{l='Ref_ID';e={$refnew}},@{l='Host';e={($_ | select -ExpandProperty ipv4addrs).host}},@{l='IPV4Addr';e={($_ | select -ExpandProperty ipv4addrs).ipv4addr}}
    $refidtest = $jsonContent.Ref_ID
    $ipaddress = $jsonContent.IPV4Addr
    $recordType = "Host"}
    Catch {throw "FAILED: Failed to convert Json Data: $($_.Exception.Message)"}
    }

ElseIf (($ARecordTest.Content) -and ($ARecordTest.Content -ne "[]")) {

    # Get Reference ID for A Record and IP Address
    Try {$jsonContent = $ARecordTest.Content | ConvertFrom-Json
    $jsonContent._ref -match "record:a/([^;]*):" | Out-Null
    $refnew = $Matches[1]
    $jsonContent = $jsonContent | select @{l='Ref_ID';e={$refnew}},@{l='Host';e={($_.name)}},@{l='IPV4Addr';e={($_.ipv4addr)}}
    $refidtest = $jsonContent.Ref_ID
...
# Return Results
If ($refidtest) {Throw "FAILED: Error occured removing Reference ID $refid for $recordType Record $($vmhostname) with IP $ipaddress on InfoBlox Server $InfoBloxServer (Record Still Exists)"}
Else {return "SUCCESS: Decommissioned Reference ID $refid for $recordType Record $($vmhostname) with IP $ipaddress on InfoBlox Server $InfoBloxServer"} 

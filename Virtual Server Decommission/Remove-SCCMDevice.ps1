<#
  .SYNOPSIS
  Removes a Device Record from the SCCM Server.

  .DESCRIPTION

  This script will remove a Device Record from the SCCM Server. If the script
  ...
  .OUTPUTS
  Remove-SCCMDevice.ps1 will output any Exception Messages if they occur.

  .EXAMPLE
  PS> .\Remove-SCCMDevice.ps1 -username XXXXX -password Password1! -JumpServer XXXXXXXXXX.domain.com -SCCMServer YYYYY.domain.com -SiteCode XXX -vmhostname YYYYY -SCCMUser ZZZZZ -SCCMPass Password1!
#>
...
If (!$Device) {return "WARNING: No Device ID found for $vmhostname in SiteCode $($SiteCode) on SCCM server $SCCMServer (Please check manually)"}

# Remove Device if Found
Try {$Device | Remove-CMDevice -Force -Confirm:$False -ErrorAction Stop}
Catch {throw "FAILED: Error occured removing $vmhostname SiteCode $($SiteCode) on SCCM server $($SCCMServer): $($_.Exception.Message)"}

# Verify Device has been Removed
Try {$DeviceTest = Get-cmdevice -Name $vmhostname -ErrorAction Stop}
Catch {$DeviceTest = $null} # This is to completely supress any errors from Get-cmdevice when device not found

# Return Results
If ($DeviceTest) {Throw "FAILED: Error occured removing Device ID $($Device.ResourceID) for $vmhostname in SiteCode $($SiteCode) on SCCM server $SCCMServer (Device Still Exists)"}
Else {return "SUCCESS: Decommissioned Device ID $($Device.ResourceID) for $vmhostname in SiteCode $($SiteCode) on SCCM server $SCCMServer"}
